...
module Googleapiop
  class Googleapiop
    ...
    def sheet_run(spredsheet_id = nil)
      spreadsheet_id = "1CeskOJWkqKEPJhIBDFOKS8i7uw7tjq3B-G2u-eyC0ac" unless spreadsheet_id
      range = "A1:F"
    ...
    def doc_run(document_id = nil)
      document_id = "195j9eDD3ccgjQRttHhJPymLJUCOUjs-jmwTrekvdjFE" unless document_id
      document = @service_docs.get_document document_id
      puts "The title of the document is: #{document.title}."
    end

    def gh_run
      stdin, stdout, stderr = *Open3.popen3('gh repo list --private --limit 300')
      list = []
      stdout.each do |line|
        ary = line.split(/\t+/)
        list << ary.shift
    ...
    def cal_run(calendar_id = nil)
      calendar_id = "primary" unless calendar_id
